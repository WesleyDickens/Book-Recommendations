...
files_list = [(f, os.path.join(cwd, f)) for f in os.listdir(cwd) if os.path.isfile(os.path.join(cwd, f))]
...
#     model.load_state_dict(torch.load('/mount/src/book-recommendations/collab_filt_model_state_dict.pth', map_location=device))
#     model.eval()
#     return model
